...
exports.killPIDs = function (pids) {
	...
	return new Promise(function (resolve, reject) {
		...
		if (process.env.VERBOSE) console.log(COLORS.magenta("[bash.origin.process] Run: " + command));
...
Runner.prototype.killAll = function () {
	...
	return new Promise(function (resolve, reject) {
		...
		return setTimeout(function () {
			try {
				console.log(COLORS.magenta("[bash.origin.process] Quitting."));
				process.stdin.end();
				process.exit(0);
				return resolve(null);
			} catch (err) {
...
Runner.prototype.runProgramProcess = function (options) {
	...
	options.cwd = (options.cwd && LIB.PATH.resolve(process.cwd(), options.cwd)) || process.cwd();
...
Runner.prototype.writeHeader = function () {
	var self = this;
	console.log(COLORS.cyan("[bash.origin.process] Enter '<ProgramNumber> + [Return]' to restart program or 'commands' (list commands) or 'q' (quit):"));
	if (Object.keys(self._bootedPrograms).length === 0) {
		console.log(COLORS.cyan("[bash.origin.process] (No programs registered)"));
		return;
	}
	Object.keys(self._bootedPrograms).forEach(function (programNumber) {
		console.log(COLORS.cyan("[bash.origin.process]  " + COLORS.bold(""+programNumber) + " - " + self._bootedPrograms[programNumber].label));
	});
}
		...
		//			}
				...
				for (var programNumber in self._bootedPrograms) {
					if (programNumber == data) {
						...
						console.log(COLORS.magenta("[bash.origin.process] Restarting program: " + self._bootedPrograms[programNumber].label));
				...
				console.log(COLORS.red("[bash.origin.process] Program with number '" + data + "' not found!"));
				return;
			} catch (err) {
...
var Program = function (API, options) {
	...
	self.kill = function () {
		if (process.env.VERBOSE) console.log(COLORS.magenta("[bash.origin.process] Killing program '" + self.label + "'"));
		...
		return new Promise(function (resolve, reject) {
			...
			try {
				...
				function waitUntilAlive () {
					...
					console.log(COLORS.magenta("[bash.origin.process] Waiting until program '" + self.label + "' is alive by checking route '" + url + "' against '" + JSON.stringify(options.routes.alive.expect || "") + "'."));
					...
					function doCheck () {
						...
						return LIB.REQUEST({
							...
							console.log(COLORS.magenta("[bash.origin.process] Program '" + self.label + "' is alive!"));
					...
					if (options.routes.alive.wait) {
						setTimeout(function () {
							doCheck();
						}, options.routes.alive.wait);
					} else {
					...
					if (!lastMessageTimeout) {
						function makeTimeout () {
							lastMessageTimeout = setTimeout(function () {
								...
								if (process.env.VERBOSE) console.log(COLORS.magenta("[bash.origin.process] Done booting program '" + self.label + "'."));
					...
					return Promise.try(async function () {
						...
						if (
							...
							if (self._options.run['.@'] === 'github.com~0ink~codeblock/codeblock:Codeblock') {
								...
								var codeblock = LIB.CODEBLOCK.compile(self._options.run, {
									ENV: env
								});
								...
								if (codeblock.getFormat() === 'bash') {
									...
									if (/^.+\sbash\.origin\.script\s*$/m.test(commands)) {
										return LIB.BASH_ORIGIN_MODULES.compile(commands, process.cwd() + "/.~" + self.label + ".codeblock.bo.sh").then(function (code) {
											return [												
												'export BO_LOADED=',
								...
								if (codeblock.getFormat() === 'pinf.it') {

									const tmpPath = LIB.PATH.join(process.cwd(), ".~" + self.label + ".codeblock.inf.json");

									await LIB.FS_EXTRA.outputFile(tmpPath, codeblock.getCode(), 'utf8');

									return `pinf.it "${tmpPath}"`;
				...
				return normalizeCommands().then(function (commands) {
					...
					console.log(COLORS.magenta("[bash.origin.process] Run program '" + self.label + "': " + commandsSummary));
					...
					function prefixAndWrite (stream, prefixColor, lines) {
						...
						if (!lines) return;
						var prefix = COLORS.bold("[" + self._number + ":" + self.label + "] ");
						if (prefixColor) {
							prefix = COLORS[prefixColor](prefix);
						}
						lines.split("\n").forEach(function (line) {
					...
					self.process.on("error", function(err) {
						process.stdout.write(COLORS.red.bold("[" + self._number + ":" + self.label + "] ERROR[1]: ") + COLORS.red(""+err.stack));
						return reject(err);
					});
				...
				process.stdout.write(COLORS.red.bold("[" + self._number + ":" + self.label + "] ERROR[2]: ") + COLORS.red(""+err.stack));
				return reject(err);
			}
