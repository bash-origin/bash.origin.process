...
const BASH_ORIGIN_MODULES = require("bash.origin.modules");
...
exports.killPIDs = function (pids) {
	...
	return new Promise(function (resolve, reject) {
		...
		if (process.env.VERBOSE) console.log(("Run: " + command).magenta);
...
Runner.prototype.runREPL = function () {
	...
	return new Promise(function (resolve, reject) {
		...
		//			}
				...
				if (process.env.VERBOSE) console.log("stdin data:", data);
	...
	return Promise.map(Object.keys(instructions), function (alias) {
		...
		function ensureDepends () {
			...
			return Promise.map(instruction.depends, function (otherAlias) {
				...
				if (process.env.VERBOSE) console.log("Waiting to boot process '" + alias + "' until process '" + otherAlias + "' is alive!");
...
var Program = function (API, options) {
	...
	if (process.env.VEROSE) console.log("Run program '" + self.label, "using config:", options.config);
	...
	self.kill = function () {
		if (process.env.VERBOSE) console.log(("Killing program '" + self.label + "'").magenta);
		...
		return new Promise(function (resolve, reject) {
			...
			try {
				...
				function waitUntilAlive () {
					...
					if (process.env.VERBOSE) console.log("waitUntilAlive()", self.label);
					...
					if (
						...
						if (process.env.VERBOSE) console.log("no routes, skip tests.", self.label);
						return resolve(null);
					}
					...
					console.log(("Waiting until program '" + self.label + "' is alive by checking route '" + url + "' against '" + JSON.stringify(options.routes.alive.expect || "") + "'.").magenta);
					...
					function doCheck () {
						return REQUEST({
							...
							if (options.routes.alive.expect) {
								...
								if (typeof options.routes.alive.expect === "string") {
									...
									try {
										...
										console.error("Error parsing response JSON to match it to expected value.", err.stack);
										return checkAgain();
									}
							...
							console.log(("Program '" + self.label + "' is alive!").magenta);

							if (options.routes.alive.exit) {

								if (process.env.VERBOSE) console.log("Trigger exit due to 'options.routes.alive.exit'");
				...
				function onNewMessage () {
					...
					if (process.env.VERBOSE) console.log("onNewMessage()", self.label);
					...
					if (!lastMessageTimeout) {
						function makeTimeout () {
							lastMessageTimeout = setTimeout(function () {
								...
								if (process.env.VERBOSE) console.log(("Done booting program '" + self.label + "'.").magenta);
									...
									if (/^.+\sbash\.origin\.script\s*$/m.test(commands)) {
										return BASH_ORIGIN_MODULES.compile(commands, process.cwd() + "/." + self.label + ".codeblock.bo.sh").then(function (code) {
											return [												
												...
												'export BO_ROOT_SCRIPT_PATH="' + PATH.join(__dirname, "../node_modules/bash.origin/bash.origin") + '"',
												code,
												'as "__codeblock__"'
											].join("\n");
										});
				...
				return normalizeCommands().then(function (commands) {
					...
					if (process.env.VERBOSE) console.log("Run commands:", commands, {
						cwd: options.cwd,
						env: options.env || {}
					});
					...
					console.log(("Run program '" + self.label + "': " + commandsSummary).magenta);
					...
					if (process.env.VERBOSE) console.log("Write commands:", commands);
			...
			if (process.env.VERBOSE) console.error("CAUGHT ERROR", err.stack, "Shutting down process.");
